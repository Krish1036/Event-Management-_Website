...
interface AdminEmailCenterClientProps {
  user: User;
  profile: Profile;
  events: Event[];
...
export default function AdminEmailCenterClient({
  user,
  profile,
  events,
  emailHistory,
  selectedEventId
}: AdminEmailCenterClientProps) {
  ...
  const fetchRegistrations = async () => {
    try {
      const response = await fetch(`/api/admin/events/${selectedEvent}/registrations`);
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const response = await fetch('/api/admin/events/send-email', {
        method: 'POST',
        headers: {
