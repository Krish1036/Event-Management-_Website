...
export async function POST(request: NextRequest) {
  try {
    ...
    if (pricingType === 'custom' && Array.isArray(pricingOptions) && pricingOptions.length > 0) {
      ...
      const { error: pricingOptionsError } = await supabase
    ...
    if (pricingType === 'custom') {
      logDetails.pricing_type = 'custom';
      logDetails.pricing_dropdown_label = eventInput.pricing_dropdown_label;
      logDetails.pricing_options_count = pricingOptions.length;
    } else {
      logDetails.pricing_type = pricingType;
      if (pricingType === 'paid') {
        logDetails.price = price;
      }
    }

    const { error: logError } = await supabase.from('admin_logs').insert({
      admin_id: user.id,
      action: pricingType === 'custom' ? 'CREATE_CUSTOM_PRICING_EVENT' : 'CREATE_EVENT',
      details: logDetails
    });
