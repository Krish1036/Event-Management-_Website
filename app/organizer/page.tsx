  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (profile.role !== 'organizer') {
        ...
        throw new Error('You are not authorized as an organizer');
      }

      toast.success('Organizer login successful');
      router.push('/organizer-dashboard');
    } catch (err: any) {
      toast.error(err.message ?? 'Organizer login failed');
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4">
      <div className="w-full max-w-md">
        <div className="mb-8 text-center">
          <h1 className="text-3xl font-bold text-white mb-2">Organizer Login</h1>
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-6">
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="block text-sm font-medium text-slate-200 mb-2">Organizer Email</label>
              <input
                type="email"
                required
                value={email}
          ...
          <div className="mt-6 pt-6 border-t border-slate-800">
            <p className="text-center text-xs text-slate-500">Organizer access only. Unauthorized access will be logged.</p>
          </div>
        </div>
      </div>
    </div>
